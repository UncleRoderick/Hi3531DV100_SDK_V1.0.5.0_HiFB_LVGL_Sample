/******************************************************************************
  Some simple Hisilicon Hi35xx ivs functions.
...
HI_S32 SAMPLE_COMM_IVS_SoftFrmObj(IVE_IMAGE_S *pstImg, IVE_RECT_U16_S *pstRect, HI_U32 u32RectNum)
{	
    HI_U16 i, r;

	SAMPLE_CHECK_EXPR_RET(pstImg==HI_NULL, HI_FAILURE, "pstImg can't be NULL!\n");
	SAMPLE_CHECK_EXPR_RET(pstRect==HI_NULL, HI_FAILURE, "pstRect can't be NULL!\n");
	SAMPLE_CHECK_EXPR_RET(pstImg->pu8VirAddr[0]==HI_NULL, HI_FAILURE, "pstImg->pu8VirAddr[0] can't be NULL!\n");
    
	for(i=0; i<u32RectNum; i++)
	{
		HI_U16 u16X = pstRect[i].u16X;
		HI_U16 u16Y = pstRect[i].u16Y;
		HI_U16 u16W = pstRect[i].u16Width;
		HI_U16 u16H = pstRect[i].u16Height;		

		// top
		HI_U8 *pu8 = pstImg->pu8VirAddr[0] + pstImg->u16Stride[0] * u16Y;		
		memset(&pu8[u16X], 255, u16W);

		// bottom
		pu8 = pstImg->pu8VirAddr[0] + pstImg->u16Stride[0] * (u16Y + u16H);
		memset(&pu8[u16X], 255, u16W);

		// left and right
		for(r=u16Y; r<u16Y+u16H; r++)
		{
			pu8 = pstImg->pu8VirAddr[0] + pstImg->u16Stride[0] * r;
			pu8[u16X] = 255;
			pu8[u16X + u16W] = 255;
		}
	}
